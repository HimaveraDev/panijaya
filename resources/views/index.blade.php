...
@section('content')
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 pt-20">
            <div class="max-w-2xl text-white">
                ...
                <div class="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4">
                    <a href="/katalog"
                        class="px-8 py-4 bg-wood-600 hover:bg-wood-700 text-white font-semibold rounded-full text-center transition-all transform hover:scale-105">Lihat
                        Produk</a>
                    <a href="/tentang"
                        class="px-8 py-4 bg-white/10 hover:bg-white/20 backdrop-blur-md text-white border border-white/30 font-semibold rounded-full text-center transition-all">Selengkapnya</a>
                </div>
            </div>
        </div>
    </section>
        ...
        <section class="py-12 bg-white border-b border-gray-100">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="flex flex-wrap justify-center gap-6">
                    @if(isset($siteSettings->marketplace_links['shopee']))
                        <a href="{{ $siteSettings->marketplace_links['shopee'] }}" target="_blank" rel="noopener"
                            class="flex items-center px-8 py-4 bg-white border-2 border-orange-100 hover:border-orange-500 rounded-xl shadow-sm hover:shadow-lg transition-all transform hover:-translate-y-1 group">
                            <div
                                class="w-14 h-14 bg-orange-50 text-orange-500 rounded-full flex items-center justify-center mr-4 group-hover:bg-orange-100 transition-colors">
                                <img src="https://img.icons8.com/color/480/shopee.png" class="w-10 h-10 object-contain"
                                    alt="Shopee">
                            </div>
                            <div>
                    ...
                    @if(isset($siteSettings->marketplace_links['tokopedia']))
                        <a href="{{ $siteSettings->marketplace_links['tokopedia'] }}" target="_blank" rel="noopener"
                            class="flex items-center px-8 py-4 bg-white border-2 border-green-100 hover:border-green-500 rounded-xl shadow-sm hover:shadow-lg transition-all transform hover:-translate-y-1 group">
                            <div
                                class="w-14 h-14 bg-green-50 text-green-500 rounded-full flex items-center justify-center mr-4 group-hover:bg-green-100 transition-colors">
                                <img src="https://www.freepnglogos.com/uploads/logo-tokopedia-png/berita-tokopedia-info-berita-terbaru-tokopedia-6.png"
                                    class="w-10 h-10 object-contain" alt="Tokopedia">
                            </div>
                            <div>
                                <p class="text-xs text-gray-500 uppercase tracking-widest font-bold mb-1">Official Store</p>
                                <p class="text-lg font-bold text-gray-900 group-hover:text-green-500 transition-colors">Tokopedia
                                </p>
                            </div>
                        </a>
                    @endif
                    ...
                    @if(isset($siteSettings->marketplace_links['tiktok']))
                        <a href="{{ $siteSettings->marketplace_links['tiktok'] }}" target="_blank" rel="noopener"
                            class="flex items-center px-8 py-4 bg-white border-2 border-gray-200 hover:border-black rounded-xl shadow-sm hover:shadow-lg transition-all transform hover:-translate-y-1 group">
                            <div
                                class="w-14 h-14 bg-gray-50 text-gray-900 rounded-full flex items-center justify-center mr-4 group-hover:bg-gray-200 transition-colors">
                                <img src="https://img.icons8.com/color/480/tiktok--v1.png" class="w-10 h-10 object-contain"
                                    alt="TikTok Shop">
                            </div>
                            <div>
                                <p class="text-xs text-gray-500 uppercase tracking-widest font-bold mb-1">Official Store</p>
                ...
                <div class="mt-12 pt-8 border-t border-gray-100">
                    <p class="text-center text-sm font-semibold text-gray-900 uppercase tracking-widest">Mendukung Berbagai
                        Metode Pembayaran Aman</p>
                    ...
                    <div class="swiper-wrapper flex">
                        @foreach($testimonials as $testimonial)
                            <div class="swiper-slide h-auto">
                                <div
                                    class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden h-full flex flex-col hover:shadow-md transition-all duration-300">
                                    <div class="w-full h-48 bg-gray-50 p-2">
                                        <img src="{{ $testimonial->image_url }}" class="w-full h-full object-contain rounded-lg"
                                    ...
                                    <div class="p-4 flex flex-col flex-grow relative">
                                        ...
                                        <p
                                            class="text-gray-600 text-[0.8rem] leading-snug mb-3 flex-grow italic line-clamp-3 min-h-[3rem]">
                                            {{ \Illuminate\Support\Str::words($testimonial->content, 9, '...') }}</p>
                                        <div class="mt-auto pt-3 border-t border-gray-50">
                                            <h4 class="text-xs font-bold text-gray-900 truncate">{{ $testimonial->name }}</h4>
                                            <p class="text-wood-600 text-[10px] font-medium truncate">
                                                {{ $testimonial->role ?? 'Pelanggan Setia' }}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                <div class="swiper-wrapper flex">
                    @foreach($categories as $category)
                        <div class="swiper-slide">
                            <a href="/katalog?kategori={{ $category->slug }}"
                                class="group relative block w-full h-80 rounded-2xl overflow-hidden shadow-lg transition-all hover:-translate-y-2">
                                ...
                                <div class="absolute bottom-0 left-0 p-6">
                                    ...
                                    <p class="text-wood-400 text-sm opacity-0 group-hover:opacity-100 transition-opacity">Lihat
                                        Koleksi &rarr;</p>
                                </div>
                            </a>
                        </div>
            ...
            <div class="flex flex-col lg:flex-row items-center gap-16">
                <div class="w-full lg:w-1/2">
                    <h2 class="text-3xl md:text-4xl font-bold text-gray-900 mb-8">
                        {{ $siteSettings->features_title ?? 'Mengapa Memilih Pani Jaya?' }}</h2>
                        ...
                        @if($siteSettings->feature_3_title)
                            <div class="flex items-start">
                                <div class="flex-shrink-0 bg-wood-100 p-3 rounded-lg text-wood-600">
                                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M12 16V15m0 1v-8m0 0h.01">
                                        </path>
                                    </svg>
                                </div>
                                <div class="ml-4">
                ...
                <div class="w-full lg:w-1/2 relative">
                    <img src="{{ $siteSettings->features_image_url }}" class="rounded-3xl shadow-2xl relative z-10"
                        alt="Features">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                @foreach($featuredProducts as $product)
                    <div
                        class="bg-white rounded-xl border border-gray-100 overflow-hidden group shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col h-full">
                        <div class="relative h-64 overflow-hidden">
                            ...
                            <div class="absolute top-4 left-4">
                                <span
                                    class="bg-wood-600 text-white text-[10px] px-2 py-1 rounded uppercase tracking-widest font-bold">Featured</span>
                            </div>
                        </div>
                        <div class="p-6 flex flex-col flex-grow">
                            <p class="text-[10px] text-wood-600 font-bold uppercase tracking-widest mb-1">
                                {{ $product->category->name }}</p>
                            <h3 class="text-lg font-bold text-gray-900 mb-4 line-clamp-2 min-h-[3.5rem]">{{ $product->name }}
                            </h3>
                            <div class="mt-auto">
                                <a href="/katalog/{{ $product->slug }}"
                                    class="block w-full text-center py-3 bg-gray-50 hover:bg-wood-600 hover:text-white text-gray-900 font-semibold rounded-lg transition-colors text-sm">Detail
                                    Produk</a>
                            </div>
                        </div>
                    </div>
    ...
    <section class="py-20 bg-gray-900 overflow-hidden relative">
        <div class="max-w-4xl mx-auto text-center px-4 relative z-10">
            ...
            <p class="text-gray-400 text-lg mb-10">Tanya jawab seputar material, ukuran, hingga perkiraan harga secara
                gratis melalui WhatsApp.</p>
            <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $siteSettings->whatsapp_number) }}"
                class="inline-flex items-center px-10 py-5 bg-green-500 hover:bg-green-600 text-white font-bold rounded-full text-lg shadow-xl transition-all transform hover:scale-105">
        ...
        <!-- Abstract Decoration -->
        <div
            class="absolute top-0 right-0 -translate-y-1/2 translate-x-1/2 w-96 h-96 bg-wood-600 rounded-full blur-[120px] opacity-20">
        </div>
    </section>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-10">
                @foreach($latestArticles as $article)
                    <article
                        class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden group flex flex-col h-full">
                        ...
                        <div class="p-8 flex flex-col flex-grow">
                            <span
                                class="text-wood-600 font-bold text-[10px] uppercase tracking-widest">{{ $article->published_at?->format('d M Y') }}</span>
                            <h3
                                class="text-lg font-bold mt-2 mb-4 group-hover:text-wood-600 transition-colors leading-tight line-clamp-3 min-h-[4.5rem]">
                                <a href="/artikel/{{ $article->slug }}">{{ $article->title }}</a>
                            </h3>
                            <p class="text-gray-500 text-sm line-clamp-2 mb-6 min-h-[2.5rem]">
                                {{ strip_tags($article->content) }}</p>
                            <div class="mt-auto">
                                <a href="/artikel/{{ $article->slug }}"
                                    class="text-gray-900 font-bold text-sm border-b-2 border-wood-500 pb-1 hover:text-wood-600 transition-colors">Baca
                                    Selengkapnya</a>
                            </div>
                        </div>
                    </article>
